...
# Modified for integration with SumoHub 2019-10-02
# ====================================================================================================
...
class Jassa:
    ...
    def multiquery(self, IDs):
        # Adding "--" IDKWhy
        multi_fasta_seq = {
            ID.split("\n")[0][1::]: ID.split("\n")[1].join(["--", "--"]) for ID in IDs
        }
        out = self.jassa(multi_fasta_seq)
        return out

    def jassa(self, multi_fasta_seq):
        table_beauclair2014_sim = self.table_beauclair2014_sim
        table_beauclair2012_f3_all = self.table_beauclair2012_f3_all
        table_beauclair2012_f3_con = self.table_beauclair2012_f3_con
        table_beauclair2012_f3_inv = self.table_beauclair2012_f3_inv
        list_aa = self.list_aa
        liste_seq_consensus_sumo_default = self.liste_seq_consensus_sumo_default
        ...
        for fasta_seq in multi_fasta_seq.items():
            name_seq = fasta_seq[0]
            nb_seq += 1
            seq = fasta_seq[1].upper()
            ...
            if not list_K:
                ...
                for key in table.keys():
                    if key != "Seq_name":
                        ...
                        table[key].extend(name_seq)

                        # table["Seq_Name"].extend(name_seq)
                        # table["Pos_K"]"*"
            else:
                for pos_K in list_K:
                    # Ask. We can find K in the N-terminal región [0:3] and we don't know what to do with them
                    motif_5 = (
                        seq[pos_K - 2]
                        + seq[pos_K - 1]
                        + seq[pos_K]
                    ...
                    db_hit_con = liste_seq_consensus_sumo_default.count(
                        motif_5[1:5] + ":"
                    )
                    db_hit_inv = liste_seq_consensus_sumo_default.count(
                        ":" + motif_5[0:4]
                    )

                    # Need to analize this code
                    # if db_hit_con!=0 or db_hit_inv!=0 or score_best_con!="None" or score_best_inv!="None":
                    # 	print("K {:4d}\tMotif {}\tScore Cons.: {}\tScore Inv.: {}\tDB Hit Cons.: {:4d}\tDB Hit Inv.: {:4d}".format(pos_K-1,motif_5,score_best_con,score_best_inv,db_hit_con,db_hit_inv))

                    table["Seq_Name"].extend([name_seq])
                    table["Pos_K"].extend([pos_K - 1])
                    table["Motif"].extend([motif_5])
                    table["DB_All_Score_Cons"].extend([score_all[0]])
                    table["DB_All_Score_Inv"].extend([score_all[1]])
                    table["DB_All_Status_Cons"].extend([score_all[2]])
                    table["DB_All_Status_Inv"].extend([score_all[3]])
                    table["DB_Cons_Score_Cons"].extend([score_con[0]])
                    table["DB_Cons_Score_Inv"].extend([score_con[1]])
                    table["DB_Cons_Status_Cons"].extend([score_con[2]])
                    table["DB_Cons_Status_Inv"].extend([score_con[3]])
                    table["DB_Inv_Score_Cons"].extend([score_inv[0]])
                    table["DB_Inv_Score_Inv"].extend([score_inv[1]])
                    table["DB_Inv_Status_Cons"].extend([score_inv[2]])
                    table["DB_Inv_Status_Inv"].extend([score_inv[3]])
                    table["DB_Hit_Cons"].extend([db_hit_con])
                    table["DB_Hit_Inv"].extend([db_hit_inv])
        return table
...
if __name__ == "__main__":
    ...
    with open("sumohub/cache/IDs_FASTA.pickle", "rb") as fread:
        IDs = pickle.load(fread)

    jassa = Jassa()
    out = jassa.multiquery(IDs)
